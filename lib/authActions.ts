...
export async function signupWithEmail(
  ...
  await ensureUserDoc(cred.user, { username, profilePic });
}

// Returns whether this was a brand-new account, so the caller can decide
// whether to show onboarding (avatar/username picker) — same as the old
// __doGoogle / __doGithub `isNew` flag.
export async function loginWithGoogle(): Promise<{ isNew: boolean }> {
  const provider = new GoogleAuthProvider();
  const cred = await signInWithPopup(auth, provider);
  const { created } = await ensureUserDoc(cred.user);
  return { isNew: created };
}

export async function loginWithGithub(): Promise<{ isNew: boolean }> {
