...
interface AuthModalProps {
  open: boolean;
  onClose: () => void;
  /** Called after a successful Google/GitHub sign-in that created a brand-new account,
   *  so the parent can open the OAuth onboarding (username/avatar) modal next. */
  onNewOAuthUser?: () => void;
}

type Tab = "login" | "signup";

export default function AuthModal({ open, onClose, onNewOAuthUser }: AuthModalProps) {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      await signupWithEmail(signupUsername, signupEmail, signupPassword, signupAvatar);
      onClose();
      // Reset form
      setSignupUsername("");
      setSignupEmail("");
  ...
  async function handleGithub() {
    setOauthError("");
    try {
      const { isNew } = await loginWithGithub();
      onClose();
      if (isNew) onNewOAuthUser?.();
    } catch (err: any) {
      setOauthError(friendlyAuthError(err?.code));
